...
def init_database():
    ...
    client = Clients(client_key="yopi", client_secret=hash_pass, status=1, salt=salt)
    clientnonInternal = Clients(client_key="ragil", client_secret=hash_pass, status=0, salt=salt)
    db.session.add(client)
    db.session.add(clientnonInternal)
    db.session.commit()
    
    user=Users(client_id=1, user_name="Little Joni", age=23, sex="male")
    db.session.add(user)
    db.session.commit()
